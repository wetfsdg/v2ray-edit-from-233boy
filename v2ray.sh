...
update_v2ray.sh() {
	if [[ $_test ]]; then
		local latest_version=$(curl -H 'Cache-Control: no-cache' -s -L "https://raw.githubusercontent.com/rootmelo92118/v2ray-edit-from-233boy/test/v2ray.sh" | grep '_version' -m1 | cut -d\" -f2)
	else
		local latest_version=$(curl -H 'Cache-Control: no-cache' -s -L "https://raw.githubusercontent.com/rootmelo92118/v2ray-edit-from-233boy/master/v2ray.sh" | grep '_version' -m1 | cut -d\" -f2)
	fi
